...
class GitHubRoleProcessor:
    ...
    def get_user_role(username: str, email: str) -> str:
        ...
        logger.info(f"Assigned default role '{default_role}' to GitHub user '{username}'")
        return default_role
...
def exchange_github_code_for_token(code: str, redirect_uri: str) -> str:
    ...
    if 'error' in token_info:
        ...
        raise Exception(f"GitHub token error: {token_info.get('error_description', token_info.get('error'))}")
...
def get_github_user_info(access_token: str) -> Dict:
    ...
    if user_response.status_code != 200:
        ...
        raise Exception(f"Failed to fetch user information: {user_response.status_code}")
    ...
    if not user_info.get('email'):
        ...
        emails_response = requests.get('https://api.github.com/user/emails', headers=headers, timeout=30)
...
def callback(provider: str):
    ...
    try:
        ...
        if not received_state or not stored_state or received_state != stored_state:
            logger.error(f"State mismatch: received={received_state}, stored={stored_state}")
        ...
        if provider == 'github':
            ...
            logger.info(f"GitHub user authenticated: {user_info.get('login')} ({user_info.get('email')})")
...
def debug_session():
    ...
    return jsonify({
        ...
        'oauth_states': {k: v for k, v in session.items() if k.startswith('oauth_state_')},
        'config': {
