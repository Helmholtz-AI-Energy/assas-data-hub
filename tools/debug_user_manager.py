    ...
    try:
        ...
        if direct_count > 0:
            ...
            for i, user in enumerate(users_collection.find().limit(3)):
                print(f"  {i+1}. {user.get('username', 'no username')} - {user.get('email', 'no email')}")
            ...
            def get_user_stats_test():
                ...
                try:
                    ...
                    stats = {
                        ...
                        'github_users': sum(1 for user in all_users if user.get('provider') == 'github'),
                        'basic_auth_users': sum(1 for user in all_users if user.get('provider') == 'basic_auth'),
                        'admin_users': sum(1 for user in all_users if 'admin' in user.get('roles', [])),
                    }
                    ...
                    return {
                        'total_users': 0, 'active_users': 0, 'github_users': 0,
                        'basic_auth_users': 0, 'admin_users': 0
                    }
                ...
                for collection_name in collections:
                    ...
                    if count > 0 and 'user' in collection_name.lower():
                        ...
                        if sample:
                            ...
                            print(f"        Sample: {sample.get('username', 'no username')} - {sample.get('email', 'no email')}")
