...
def check_session_config():
    ...
    try:
        ...
        print(f"\nConfig values:")
        print(
            f"SECRET_KEY: {'Set' if hasattr(config, 'SECRET_KEY') and config.SECRET_KEY else 'NOT SET'}"
        )
        print(
            f"PERMANENT_SESSION_LIFETIME: {getattr(config, 'PERMANENT_SESSION_LIFETIME', 'Not set')}"
        )
        ...
        print(
            f"SESSION_COOKIE_SECURE: {getattr(config, 'SESSION_COOKIE_SECURE', 'Not set')}"
        )
        print(
            f"SESSION_COOKIE_HTTPONLY: {getattr(config, 'SESSION_COOKIE_HTTPONLY', 'Not set')}"
        )
        print(
            f"SESSION_COOKIE_SAMESITE: {getattr(config, 'SESSION_COOKIE_SAMESITE', 'Not set')}"
        )
