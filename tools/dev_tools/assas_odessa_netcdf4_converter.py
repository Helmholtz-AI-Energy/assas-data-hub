...
class AssasOdessaNetCDF4Converter:
    ...
    def parse_variable_from_odessa_in_vessel(
        ...
        for channel_number in range(vessel.len("CHANNEL")):
            ...
            for mesh_number in range(channel.len("MESH")):
                ...
                logger.debug(
                    f"Channel number {channel_number}, Mesh number {mesh_number}."
                )
                ...
                logger.debug(
                    f"Collect variable structure {variable_structure}, extract data point: {variable_structure[2]}."
                )
    ...
    def convert_astec_variables_to_netcdf4(
        self, output_file: str = "dataset.h5"
    ) -> None:
        """
        ...
        logger.info(
            f"Read following time_points from ASTEC archive: {self.time_points}."
        )

        with netCDF4.Dataset(
            f"{self.output_path}/{output_file}", "w", format="NETCDF4"
        ) as ncfile:
            ...
            for idx, time_point in enumerate(self.time_points):
                ...
                for _, variable in self.variable_index.iterrows():
                    ...
                    data_per_timestep = strategy_function(
                        odessa_base=odessa_base, variable_name=variable["name"]
                    )

                    logger.debug(
                        f"Read data for {variable['name']} with shape {data_per_timestep.shape}."
                    )
                    logger.debug(
                        f"Resize dataset to ({len(self.time_points)},{data_per_timestep.shape[0]},{data_per_timestep.shape[1]})."
                    )

                    variable_datasets[variable["name"]][idx, :, :] = data_per_timestep


def parse_timepoint_argument(argument_string: str) -> List[int]:
    """
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f"{args.output}/{timestamp}_{Path(__file__).stem}.log", "w"
            ),
        ],
    )

    astec_python_location = os.path.join(
        args.root, "odessa", "bin", args.computer + "-release", "wrap_python"
    )
    ...
    AssasOdessaNetCDF4Converter(
        input_path=args.input, output_path=args.output, time_points=time_points
